...
app.logUserOut = function(redirectUser){
  ...
  app.client.request(undefined,'api/logout','GET',queryStringObject,undefined,function(statusCode,responsePayload){
...
app.editMenuIteminShoppingCart = function (id, increment) {
  var payload = {
    'menuItemId' : id,
    'quantity' : increment
  };

  app.client.request(undefined,'api/shoppingcart','PUT',undefined,payload,function(statusCode,responsePayload){
    if(statusCode == 200){
      // If the request comes back as something other than 200, redirect back to dashboard
      window.location = '/myshopping/cart';
    } else {
      app.logUserOut();
    }
  });
        ...
        if(statusCode == 200){
          ...
          if(menuItems.length > 0){
            ...
            menuItems.forEach(function(menuItem){
              ...
              td0.innerHTML = menuItem.id;
              td1.innerHTML = menuItem.description;
              td2.innerHTML = menuItem.price;
              td3.innerHTML = '<a onclick="app.addMenuItem(\''+menuItem.id+'\')">Add to Shopping cart</a>';
            });
...
app.loadShoppingCart = function(){
    ...
    app.client.request(undefined,'api/shoppingcart','GET',undefined,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
        var menuItems = typeof(responsePayload.shoppingcart) == 'object' && responsePayload.shoppingcart instanceof Array && responsePayload.shoppingcart.length > 0 ? responsePayload.shoppingcart : [];
          if(menuItems.length > 0){
            ...
            menuItems.forEach(function(menuItem){
              ...
              td4.innerHTML = '<a onclick="app.editMenuIteminShoppingCart(\''+menuItem.id+'\','+(menuItem.quantity+1)+')">+1</a>';
              td5.innerHTML = '<a onclick="app.editMenuIteminShoppingCart(\''+menuItem.id+'\','+(menuItem.quantity-1)+')">-1</a>';
            });
  
            document.getElementById("totalText").style.display = 'block';
            document.getElementById("totalamount").innerHTML = responsePayload.totalAmount;
