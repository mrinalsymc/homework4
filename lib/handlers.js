            ...
            handlers.verifyToken(id).then(parsedTokenObject => {
                ...
                _data.read('menus', 'menu').then(parsedMenuItems => {
                    ...
                    _data.read('users', parsedTokenObject.email).then(parsedUserObject => {
                        ...
                        orderReceipt.items = printableShoppingCart;
                        orderReceipt.totalAmount = totalAmount/100;
                        orderReceipt.email = parsedTokenObject.email;
                        orderReceipt.time = Date.now();

                        helpers.processPayment(totalAmount, sourceToken).then(() => {
                            var orderFileName = helpers.createRandomString(20);
                            _data.create('orders', orderFileName, orderReceipt).then(_ => {
                                debug('orderReceipt was created successfully:', orderFileName);
                            })
